...
class OrderedBasicBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        out = self.conv2(out)
        return (torch.add(x if self.equalInOut else self.convShortcut(x), out), mask, features)
...
class CustomODWideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
        super(CustomODWideResNet, self).__init__()
    ...
    def forward(self, x, mask, features, is_feat):
        out = self.conv1(x)
        f0 = out
        out, mask, features = self.block1((out, mask, features))
        f1 = out
        out, mask, features = self.block2((out, mask, features))
        f2 = out
        out, mask, features = self.block3((out, mask, features))
        f3 = out
...
def CustomODwrn(**kwargs):
    """
    Constructs a Wide Residual Networks.
    """
    model = CustomODWideResNet(**kwargs)
    return model


def CustomODwrn_40_2(**kwargs):
    model = CustomODWideResNet(depth=40, widen_factor=2, **kwargs)
    return model


def CustomODwrn_40_1(**kwargs):
    model = CustomODWideResNet(depth=40, widen_factor=1, **kwargs)
    return model


def CustomODwrn_16_2(**kwargs):
    model = CustomODWideResNet(depth=16, widen_factor=2, **kwargs)
    return model


def CustomODwrn_16_1(**kwargs):
    model = CustomODWideResNet(depth=16, widen_factor=1, **kwargs)
    return model

def CustomODwrn_22_8(**kwargs):
    model = CustomODWideResNet(depth=22, widen_factor=8, **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    net_t = ODwrn_22_8(num_classes = 100)
    net_t.apply(lambda m: setattr(m, 'width_mult', 0.5))

    net = CustomODwrn_22_8(num_classes=100)
